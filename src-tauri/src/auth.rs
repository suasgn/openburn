    ...
    thread::spawn(move || {
        ...
        loop {
            if cancel_flag_thread.load(Ordering::SeqCst) {
                let _ = sender.send(Err(BackendError::Provider("OAuth cancelled".to_string())));
                return;
            }
            ...
            let request = match server.recv_timeout(poll_interval) {
                ...
                Ok(None) => continue,
                Err(err) => {
                    let _ = sender.send(Err(BackendError::Provider(format!(
            ...
            let parsed = Url::parse(&url).map_err(|err| {
                BackendError::Provider(format!("OAuth callback URL invalid: {err}"))
            });
