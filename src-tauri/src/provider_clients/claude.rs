...
pub async fn refresh_credentials(refresh_token: &str) -> Result<ClaudeCredentials> {
    ...
    if !status.is_success() {
        ...
        let message = if body.is_empty() {
            format!("Claude OAuth refresh failed: HTTP {status}")
        } else {
            format!("Claude OAuth refresh failed: HTTP {status} - {body}")
        };
    ...
    let token = response
        .json::<TokenResponse>()
        .await
        .map_err(|err| BackendError::Provider(format!("Claude OAuth decode failed: {err}")))?;
