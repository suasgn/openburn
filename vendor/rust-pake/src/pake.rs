...
mod precision_integer {
    ...
    pub fn deserialize<'de, D: Deserializer<'de>>(deserializer: D) -> Result<BigInt, D::Error> {
        BigInt::from_str(Box::<RawValue>::deserialize(deserializer)?.get())
...
// croc's Go PAKE implementation hashes BigInt.Bytes(), which is the
// unsigned magnitude without a sign-extension byte.
fn bigint_to_unsigned_bytes_be(num: &BigInt) -> Vec<u8> {
    if num == &BigInt::zero() {
        vec![]
    } else {
        num.to_bytes_be().1
    ...
    pub fn update(&mut self, key: PakePubKey) -> Result<(), Error> {
        ...
        match self.pub_pake.role {
            Role::Sender => {
                ...
                hasher.write_all(&bigint_to_unsigned_bytes_be(&x_u)).unwrap();
                hasher.write_all(&bigint_to_unsigned_bytes_be(&x_v)).unwrap();
                hasher.write_all(&bigint_to_unsigned_bytes_be(&y_u)).unwrap();
                hasher.write_all(&bigint_to_unsigned_bytes_be(&y_v)).unwrap();
                hasher.write_all(&bigint_to_unsigned_bytes_be(&z_u)).unwrap();
                hasher.write_all(&bigint_to_unsigned_bytes_be(&z_v)).unwrap();
                let digest = hasher.finalize();
                self.k = Some(digest[..].try_into().expect("Wrong length"));
...
#[cfg(test)]
mod tests {
    use super::bigint_to_unsigned_bytes_be;
    use num_bigint_dig::BigInt;

    #[test]
    fn hashes_positive_bigints_without_signed_extension() {
        assert_eq!(bigint_to_unsigned_bytes_be(&BigInt::from(0x80u32)), vec![0x80]);
    }
}
