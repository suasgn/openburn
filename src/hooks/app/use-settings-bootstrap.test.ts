...
describe("useSettingsBootstrap", () => {
  ...
  it("falls back to default time format mode when loading fails", async () => {
    const timeFormatError = new Error("time format unavailable")
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {})
    loadTimeFormatModeMock.mockRejectedValueOnce(timeFormatError)
    const args = createArgs()
    ...
    await waitFor(() => {
      expect(errorSpy).toHaveBeenCalledWith(
        "Failed to load time format mode:",
        timeFormatError
      )
